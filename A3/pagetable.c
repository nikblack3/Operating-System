...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if (frame == -1) { // Didn't find a free page.
		...
		// swap victim from mem to swap file if is not currently on swap or is
		// dirty
		if ((victim->frame & PG_DIRTY) || !(victim->frame & PG_ONSWAP)) {
			int new_swap_off = swap_pageout(frame, victim->swap_off);
			if (new_swap_off == INVALID_SWAP) {
				perror("allocate_frame: swap_pageout");
				exit(-1);
			} else {
...
char *find_physpage(addr_t vaddr, char type) {
	...
	// if pde in pgdir[idx] is 0, init the page of pgtbl
	if (!pgdir[idx].pde) {
	...
	if (p->frame & PG_VALID) {
		...
		// mem does not contain p so allocate frame for it
		int frame = allocate_frame(p);
		if (p->frame & PG_ONSWAP) {
			p->frame = frame << PAGE_SHIFT;
			// swap from swap file to memory and set clean
			int result = swap_pagein(frame, p->swap_off);
			if (result == 0) {
				p->frame &= ~PG_DIRTY;
			} else {
				perror("find_physpage: swap_pagein");
				exit(-1);
			}
